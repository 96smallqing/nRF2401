...
uint const TX_ADDRESS[TX_ADR_WIDTH]= {0x01,0x01,0x01,0x01,0x01}; //本地地址	  0x01,0x01,0x01,0x01,0x01　　主机地址
uint const RX_ADDRESS[RX_ADR_WIDTH]= {0x10,0x10,0x10,0x10,0x10}; //接收地址	1  0x10,0x10,0x10,0x10,0x10    从机１地址
uint const RX_ADDRESS_1[RX_ADR_WIDTH]= {0x10,0x10,0x10,0x10,0x20}; //接收地址2	  0x10,0x10,0x10,0x10,0x20    从机２地址
//***************************************NRF24L01寄存器指令*******************************************************
...
void init_NRF24L01_Send(void)
{
	...
	*/
    ...
    SCK=0;   // Spi clock line init high   SPI时钟线
	  IRQ=1;
	  SPI_Write_Buf(WRITE_REG + TX_ADDR, RX_ADDRESS, TX_ADR_WIDTH);    // 写本地地址     与接收代码的接收端地址相同
...
/*NRF24L01发送初始化
//***************************************************************************************/
void init_NRF24L01_Send_1(void)
{
   /*
		两个nrf24l01通信，需要满足3个条件相同：
		1.频道相同（设置频道寄存器RF_CH）
	...
	*/
    ...
    SCK=0;   // Spi clock line init high   SPI时钟线
	  ...
	  SPI_Write_Buf(WRITE_REG + RX_ADDR_P0, RX_ADDRESS_1, RX_ADR_WIDTH); // 写接收端地址
...
void nRF24L01_TxPacket_1(unsigned char * tx_buf)
{
	CE=0;   			//StandBy I模式
	SPI_Write_Buf(WRITE_REG + RX_ADDR_P0, TX_ADDRESS, RX_ADR_WIDTH); // 装载接收端地址
	SPI_Write_Buf(WR_TX_PLOAD, tx_buf, TX_PLOAD_WIDTH);     		// 装载数据
	SPI_RW_Reg(WRITE_REG + CONFIG, 0x0e);      						// IRQ收发完成中断响应，16位CRC，主发送
	CE=1;   			//置高CE，激发数据发送
	inerDelay_us(10);
...
void main(void)
{
	...
	uchar RxBuf1[2];				  //定义接收数据的数组
	init_NRF24L01();				  //NRF接受初始化
	init_NRF24L01_1();				  //NRF接受初始化
	uart1();
	UART2_config(2);
	Init_T4();       //定时器初始化
	EA = 1;           //打开全局中断
	...
	while(1)
	{
		...
		unsigned char TxBuf[20]={0x36,0x36};    //将要发送的数据放在数组中    
		unsigned char TxBuf_1[20]={0x37,0x37};    //将要发送的数据放在数组中
			jidianqi  = 1;
		init_NRF24L01();
	  SetRX_Mode();   //无按键按下为接受模式
		...
		OLED_ShowCHinese(0,3,9);//安
		OLED_ShowCHinese(18,3,10);//防
		OLED_ShowCHinese(36,3,11);//门
		OLED_ShowCHinese(54,3,12);//廊
		OLED_ShowString(72,3,temp);
		...
		if(Fasong1 ==1)
			{
				...
				init_NRF24L01_1();				  //NRF接受初始化
				SetRX_Mode();   //无按键按下为接受模式
			 nRF24L01_RxPacket(RxBuf1);
		
		}
		if(flag1 == 1)
		{
			flag1 =0;   //复位
			flag_kaisuo = 0;  //关闭开锁标志位
		...
		if(flag2 == 1)
		{
			init_NRF24L01_1();				  //NRF接受初始化
			flag2 =0;   //复位
			flag_kaisuo = 0;  //关闭中开锁标志位
			speak = !speak;  //蜂鸣器按下响，在按下不响
			init_NRF24L01_Send_1();
		  SetTX_Mode();   //有按键按下为发送模式
			nRF24L01_TxPacket_1(TxBuf_1);			 	//发送数据
			//P0=SPI_Read(STATUS);				 	//读状态寄存器的值  如果数据成功发送，那么STATUS的值应该为0x2e
			SPI_RW_Reg(WRITE_REG+STATUS,0XFF);   	//清状态寄存器
			Delay(6000);
			speak =1;               //关闭蜂鸣器
...
//三种串口发送方式
		...
		{
			if(flag_kaisuo ==1){jidianqi_flag =1;flag_kaisuo = 0;}
		...
		if((strstr((char*)RX2_Buffer,"n")!=NULL)||(strstr((char*)RX2_Buffer,"f")!=NULL))
		 {
			 jidianqi_flag =1;
			 SendString(RX2_Buffer);
			 Delay(60);
			  clear();	
